...
const ERROR_MESSAGES = {
  ...
  OLLAMA_BROWSER_ACCESS_DENIED: 'was denied local-network access by this browser. Allow Local Network Access for this FounderLab Preview, reload, then try again.',
  OLLAMA_BROWSER_ACCESS_BLOCKED: 'was blocked by this browser before local Ollama could respond. Open FounderLab in a normal browser tab, allow Local Network Access if prompted, reload, then try again.',
...
export function classifyAIError({ provider, status, code, message } = {}) {
  ...
  const resolvedStatus = Number.isInteger(status)
    ...
    : resolvedCode === 'AUTHENTICATION_REQUIRED' || resolvedCode === 'AUTHENTICATION_INVALID' ? 401
      ...
      : resolvedCode === 'REQUEST_INVALID' || resolvedCode === 'INVALID_MODEL' || resolvedCode === 'GEMINI_REQUEST_INVALID' || resolvedCode === 'GEMINI_BILLING_OR_REGION_REQUIRED' || resolvedCode === 'PROVIDER_REQUEST_TOO_LARGE' || resolvedCode === 'OLLAMA_LOCAL_ONLY' || resolvedCode === 'OLLAMA_CHAT_ONLY' || resolvedCode === 'OLLAMA_INVALID_URL' || resolvedCode === 'OLLAMA_MODEL_REQUIRED' || resolvedCode === 'OLLAMA_MODEL_UNAVAILABLE' ? 400
